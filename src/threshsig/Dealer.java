...
public class Dealer {
	...
	private BigInteger generateStrongPrime(int bitLength, SecureRandom random) {
		while (true) {
			BigInteger p = BigInteger.probablePrime(bitLength, random);
			BigInteger pprime = p.subtract(BigInteger.ONE).shiftRight(1);
	...
	public void generateKeys(final int k, final int l) {
		...
		// Modification by Michael Eischer: Replace use of SafePrimeGen
		/* Generate a Sophie Germain prime keypair */
		// pr = generateSophieGermainPrime();
		// qr = generateSophieGermainPrime();
		p = generateStrongPrime(keysize / 2, ThreshUtil.getRandom());
		q = generateStrongPrime(keysize / 2, ThreshUtil.getRandom());
